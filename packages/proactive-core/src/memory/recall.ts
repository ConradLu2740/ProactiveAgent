...
import { tokenize, queryTerms, expandedQueryTerms, isStopToken, CJK_RE } from './tokens'
import { getIndexFor, lookupCandidates } from './inverted-index'
import { maybeArchiveExpired } from './ttl'

// 兼容外部引用（store.test.ts / scene.ts 从 './recall' 导入分词工具）
export { tokenize, queryTerms, expandedQueryTerms } from './tokens'
...
export function searchMemoriesByKeyword(request: MemorySearchRequest): MemorySearchResult {
  ...
  // M9：倒排索引缩小候选集（仅扫描包含任一查询词的 atoms），无命中回退全量，保证不漏召回
