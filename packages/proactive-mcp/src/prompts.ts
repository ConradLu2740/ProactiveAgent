...
## 两阶段引导（v0.8.0）
- **第一阶段 · 建画像**：如果 persona_get 显示尚未生成用户画像，在用户有足够行为证据（约 3-5 条明确偏好/纠正）后，用 persona 相关能力沉淀画像，让后续会话共享同一份用户画像。
- **第二阶段 · 补证据**：画像建立后，若本次会话出现新的稳定偏好/纠正，先沉淀为记忆（memory_capture），再在合适时机触发画像增量更新；不要重复记录已存在的行为规则。
- **记忆维护纪律**：画像长期不更新（超过 3 天）时，用 memory_stats 查看"距上次更新"提示，主动向用户提议做一次复查（清理过时记忆、确认待确认项、必要时重整画像）。画像超载（行数/章节过多）时优先精简合并，而不是继续追加。
